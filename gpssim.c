...
int main(int argc, char *argv[])
{
	...
	FILE *fp;
	FILE *fp_prime; //XOXOX
	fp_prime=fopen("fpprime.bin","wb");//XOXOX
	int sv;
	int neph,ieph;
...
*/
	...
	//iq_buff_prime = calloc(4 * iq_buff_size, 4); //XOXOX
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		...
		for (isamp=0; isamp<iq_buff_size; isamp++)
		{
			...
			//XOXOX
			//iq_buff_prime[isamp * 2] = i_acc;
			//iq_buff_prime[isamp * 2 + 1] = q_acc;
			// Scaled by 2^7
			i_acc = (i_acc+64)>>7;
			q_acc = (q_acc+64)>>7;
		...
		else // data_format==SC16
		{
			...
			if (iumd==1)
				{
					fwrite(iq_buff, 2, 2 * iq_buff_size, fp_prime);
				}
	        }
	...
	// Free I/Q buffer
	free(iq_buff);
	//free(iq_buff_prime); //XOXOX

	// Close file
	fclose(fp);
	fclose(fp_prime); //XOXOX
